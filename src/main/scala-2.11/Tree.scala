...
trait Tree[+A] {
  ...
  override def toString: String = str("", Some(this), 0)

  def str[B](prefix: String, ot: Option[Tree[B]], depth: Int): String = ot match {
    case Some(t) => prefix + t.header + t.value.getOrElse("").toString.replace("List", "") +
      str("\n" + (" " * depth*4) + "(-)", t.left, depth + 1) +
      str("\n" + (" " * depth*4) + "(+)", t.right, depth + 1)
    case None => ""
  }

  def header: String = this match {
    case n: Node[A] => "Test "
    case l: Leaf[A] => "Answer is "
    case Empty => ""
  }
